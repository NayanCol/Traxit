...
def imgready2(image):
    ...
    upper_lab = np.array([85, 85, 85])
    # labimg = cv2.cvtColor(image, cv2.COLOR_BGR2LAB)
    # labimg[:, :, 0] = 0
    # lower_lab = np.array([0, 125, 125])
    # upper_lab = np.array([0, 135, 135])
...
def run_temperature_analysis(image):
    ...
    # cv2.imshow("separated", image)
    # cv2.waitKey(0)
    # cv2.destroyAllWindows()

    is_blur, blur_value = is_blurry(imagec, threshold=40)
    if is_blur:
    ...
    if reflection_ratio < 15 and appstr=="":
